...
#             # --- VIEWPORT MODE A: SIGN IN INTERFACE ---
#             if state['auth_mode'] == "Login":
#                 login_email_input = st.text_input("Corporate Email Address (@infor.com)", key="li_email", placeholder="name@infor.com")
#                 login_pass = st.text_input("Enter Password", type="password", key="li_pass", placeholder="••••••••")
                
#                 st.write("<br>", unsafe_allow_html=True)
                
#                 # --- FIXED: LOGIC MOVED INSIDE THE BUTTON ---
#                 if st.button("🔓 Sign In", use_container_width=True):
#                     clean_email = login_email_input.strip().lower()
                    
#                     if not clean_email.endswith("@infor.com"):
#                         st.error("❌ Access Denied: Only valid @infor.com addresses are permitted.")
#                     elif has_already_taken_test(clean_email):
#                         st.error("🚫 Sign In Restricted: This profile is locked out. Please consult your administrator.")
#                     elif authenticate_login(clean_email, login_pass):
#                         with st.spinner():
#                             state['is_logged_in'] = True
#                             state['user_email'] = clean_email
#                             state['questions'] = generate_fixed_questions()
#                             time.sleep(0.5)
#                         st.rerun()
#                     else:
        ...
        if admin_pass == ADMIN_SECRET_KEY:
            ...
            if not files:
                ...
                for file in files:
                    ...
                    col_file_name, col_dl, col_del_btn = st.columns([3, 1, 1])
                    with col_file_name:
                        st.text(f"📄 {file}")
                    with col_dl:
                        st.download_button(label="📥", data=file_data, file_name=file, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    with col_del_btn:
